...
from mysite.models import Post, resultall, fullist, choice, Comment
...
#     strSql = "SELECT * FROM mysite_resultall"
...
#             a1 = '파스타'
#             new_article=Log.objects.create(
#                 emotion=b,
#                 season=c,
#                 click= a1,
#             )
...
#     return render(request,'mysite/log.html')

def foodview(request):
    page = request.GET.get('page', '1')  # 페이지
    f2 = choice.objects.all().order_by('-pk')
    paginator = Paginator(f2, 15)  # 페이지당 10개씩 보여주기
    page_obj = paginator.get_page(page)
    context = {'f2': page_obj}
    return render(request, 'mysite/choice.html', context)
...
def fchoice(request):
    ...
    if request.method =="POST":
        res = str(df1["음식점"].values).replace("['","").replace("']","")
        if request.POST.get(f'{res}'):
            new_article=Comment.objects.create(
                고투웍= request.POST.get('고투웍'),
                구내식당= request.POST.get('구내식당'),
                돌배기집= request.POST.get('돌배기집'),
                롤링파스타= request.POST.get('롤링파스타'),
                리춘시장= request.POST.get(f'리춘시장'),
                막이오름= request.POST.get('막이오름'),
                미정국수= request.POST.get('미정국수'),
                백S비빔밥= request.POST.get('백S비빔밥'),
                백스비어= request.POST.get('백스비어'),
                백철판0410= request.POST.get('백철판0410'),
                본가= request.POST.get('본가'),
                분식9단= request.POST.get('분식9단'),
                빽다방= request.POST.get('빽다방'),
                빽보이피자= request.POST.get('빽보이피자'),
                새마을식당= request.POST.get('새마을식당'),
                성성식당= request.POST.get('성성식당'),
                역전우동0410= request.POST.get('역전우동0410'),
                연돈볼카츠= request.POST.get('연돈볼카츠'),
                원조쌈밥집 = request.POST.get('원조쌈밥집'),
                인생설렁탕= request.POST.get('인생설렁탕'),
                제순식당= request.POST.get('제순식당'),
                한신포차= request.POST.get('한신포차'),
                홍콩반점= request.POST.get('홍콩반점'),
        ...
        f3 = Comment.objects.filter().only(f'{res}')
        
      
    return render(request, f'mysite/{choice_res}map.html', {'context':f3, 'fd':fd,'f1':f1,'f2':f2 ,'count':count, 'foodlist':foodlist})
    ...
    try:
        if request.method == 'POST':
            if request.POST.get('mainphoto'):
                ...
                files = glob.glob(f'C:/temp/web_study/media/{ptlink}')
                for f in files:
...
def show_food(request):
    ...
    return render(request, 'mysite/foodlist.html')



def molar(request): #버튼 POST이었을 때 
    if request.method == 'POST':
...
def food_list1(request):
    ...
    model = load_model('C:/Program Files/VGG16_BatchNor.h5') # 모델명
    ...
    return render(request, 'mysite/food_list1.html', {'arr':arr[-1],"rec_count":rec_count, "count":count, 'emotion':emotion, 'wea':wea,'wea1':wea1, 'season':season ,'a1':a1,'b':b,'c':c, 'd':d, 'e':e, 'folist': folist, 'final':final})
